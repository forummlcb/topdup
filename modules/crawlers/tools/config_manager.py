from src.backend.lib.browser_crawler import BrowserWrapper
...
'''
    while True:
        ...
        choice = input("Press ENTER to return,\
            LINE NUMBER <=%s to edit, -LINENUMBER <=%s to remove,\
                or >%s to add new config: " %
                       (number_of_config, number_of_config, number_of_config))
        ...
        if is_digit:
            if line <= number_of_config:  # edit config
                if line > 0:  # edit old config
                    ...
                    while not validated:
                        try:
                            # note: eval is important here to get right data type
                            new_value = eval(input("New Value: "))
                            validated = True
                        except Exception as ex:
                            print(ex)
                            print(
                                "Input value in wrong format.\
                                    Remember to input like Python code\
                                        (eg. 'money' not money.\
                                            [a, b, c] not '[a, b, c]'")
...
def process_create_blank_newspaper():
    ...
    use_browser = display_yes_no_dialog(
        "Do you want to crawl using browser or not (y/n) ?")
    if use_browser:
    ...
    date_extract_type = display_choice_dialog(
        'Please choose one way to extract publish date in detail page: ', menu)
    ...
    if date_extract_type == 1:  # CSS selection
        ...
        date_xpath = input(
            "Please enter single xpath that extract html tag containing publish date: ")
    else:  # Autofind. Not implement yet
        date_xpath = ''

    xpath_count = len(webconfig.get_topics_xpath())

    remove_date_tag_html = display_yes_no_dialog(
        'Does it need to remove html tag to extract publish date (y/n)?: ')
    ...
    choice = display_choice_dialog(
        "How to treat topic that can't find its publish date ?",
        ignore_topic_menu_choice)
    if choice == 0:
        ignore_topic_not_have_publish_date = True
    else:
        ignore_topic_not_have_publish_date = False
    webconfig.set_config('ignore_topic_not_have_publish_date',
                         ignore_topic_not_have_publish_date)

    crawl_detail_choice = display_yes_no_dialog(
        'Do you want to crawl detail content (sapo, content, img) ?: ')
    ...
    if crawl_detail_choice:
        ...
        feature_image_xpath = input(
            "Please enter xpath to extract feature images url: ")

        text_xpath = display_multiple_input("Please input xpaths to get text element: ")
        image_box_xpath = display_multiple_input(
            "Please input xpaths to get image box element: ")

        image_title_xpath = display_multiple_input(
            "Please input xpaths to get title element from image box (ENTER=finish): ")
        video_box_xpath = display_multiple_input(
            "Please input xpaths to get video box element (ENTER=Finish)")
        video_title_xpath = display_multiple_input(
            "Please input xpaths to get title element from video box (ENTER=finish): ")
        audio_box_xpath = display_multiple_input(
            "Please input xpaths to get audio box element (ENTER=Finish)")
        audio_title_xpath = display_multiple_input(
            "Please input xpaths to get title element from audio box (ENTER=finish): ")

        avatar_choice_menu = ['Provide logo link', 'Xpath to get logo url']
        avatar_choice = display_choice_dialog(
            "How do you want to extract logo url ?", avatar_choice_menu)
        avatar_url = ''
        avatar_xpath = ''
    ...
    tags = display_multiple_input(
        "Please input metadata to label article\
        crawled from this config (ENTER=finish): ")
    ...
    choice = display_yes_no_dialog(
        "Do you want to run test with default config (y/n) ?")
    if choice:
...
##############################################
    ...
    if user_choice == 0:  # newspaper
        ...
        + fb_id.strip() + "/posts/?ref=page_internal"
        webconfig.set_webname(fb_name)
        webconfig.set_config('web_url', url)
        webconfig.set_config('crawl_url', url)
        webconfig.set_config('url_pattern_re', url)
    ...
    while user_choice != 3:  # finish
        clear_screen()
        print(header)
        user_choice = display_menu_dialog(
            'What do you want to do next ? ', 'Choice: ', menu)
...
###########################################
    ...
    while continue_test:
        ...
        # mean always crawl this config
        webconfig.set_minimum_duration_between_crawls(-5)
...
###########################################
    ...
    while choice != 2:
        ...
        choice = display_menu_dialog(
            'What do you want to do ?', "Choice (ENTER=cancel): ", load_config_menu)
        if choice == 0:
...
###########################################
    ...
    while not ok:
        ...
        config_file_index = display_menu_dialog(
            'Which config do you want to load ?', 'Choice (ENTER=Cancel): ', file_list)
...
###########################################
    ...
    while user_choice != len(menu) - 1:  # finish
        ...
        # newspaper_list contain all crawl config.
        # All edits will be made on newspaper_list then merge back into config_manager
        ...
        if user_choice == 0:  # add config file to list
            ...
            if filepath is not None:
                ...
                # save all changes to config_manager
                config_manager.replace_crawl_list(newspaper_list)
                config_manager.save_data()
            ...
            if choice.strip() != '' and choice.isdigit():
                ...
                input(
                    "Successfuly load %s config to working config"
            ...
            if len(newspaper_list) > 0:
                ...
                key = input(
                    '(ENTER=cancel, -config_name=remove)').strip()
                key = key.strip()
                if key != '':
                    if key[0] == '-':  # remove config
                        ...
                        input(
                            "Successfully remove %s of %s site"
                            % (key, str(count)))
                        config_manager.save_data()
            ...
            if webconfig is not None:
                ...
                input("No working site config. Use config manager to create/load one")
...
###########################################
    ...
    while choice != len(manage_config_menu) - 1:
        ...
        choice = display_menu_dialog(
            'What do you want to do ?', "Choice: ", manage_config_menu)
...
# MAIN PROGRAM
config_manager = ConfigManager(get_independent_os_path(
    ['src', 'backend', 'input', 'config.yaml']),
    get_independent_os_path(['src, ', 'backend', 'input', 'kols_list.txt']),
    get_independent_os_path(['src', 'backend', 'input', 'fb_list.txt']))
...
while main_menu_choice != 2:
    ...
    main_menu_choice = display_menu_dialog(
        'WELCOME TO CONFIG MANAGER', 'Your choice: ', main_menu)
    webconfig = None
